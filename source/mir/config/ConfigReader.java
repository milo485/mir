...
public class ConfigReader {
  ...
  public void parseFile(String aFileName, ConfigNodeBuilder aRootNode) throws ConfigFailure {
    ...
    try {
      ...
      ConfigReaderHandler handler = new ConfigReaderHandler(aRootNode, parserFactory);
  ...
  private class ConfigReaderHandler extends DefaultHandler {
    ...
    public ConfigReaderHandler(ConfigNodeBuilder aBuilder, SAXParserFactory aParserFactory) {
      super();
    ...
    public void startElement(String aUri, String aTag, String aQualifiedName, Attributes anAttributes) throws SAXException {
      ...
      try {
        if (builder==null) {
          throw new ConfigFailure("define, include and property tags cannot have content", getLocatorDescription(locator));
        }
        if (aQualifiedName.equals(propertyTagName)) {
          ...
          if (name==null) {
            throw new ConfigFailure("property has no name attribute", getLocatorDescription(locator));
          }
          else
          if (value==null) {
            throw new ConfigFailure("property \""+name+"\" has no value attribute", getLocatorDescription(locator));
          }

          builder.addProperty(name, definesManager.resolve(value, getLocatorDescription(locator)), value, getLocatorDescription(locator));
          builder=null;
          ...
          if (name==null) {
            throw new ConfigFailure("define has no name attribute", getLocatorDescription(locator));
          }
          else
          if (value==null) {
            throw new ConfigFailure("define \""+name+"\" has no value attribute", getLocatorDescription(locator));
          }

          definesManager.addDefine(name, definesManager.resolve(value, getLocatorDescription(locator)));
          builder=null;
        }
          ...
          includeFile(definesManager.resolve(fileName, getLocatorDescription(locator)));
          builder=null;
        }
        else
        {
          builder=builder.makeSubNode(aQualifiedName, getLocatorDescription(locator));
        }
      }
      catch (ConfigFailure e) {
    ...
    public void endElement(String aUri, String aTag, String aQualifiedName) throws SAXParseException {
  ...
  private class DefinesManager {
    ...
    public String resolve(String anExpression, String aLocation) throws ConfigFailure {
          ...
          if (anExpression.charAt(position+1) == '{') {
            ...
            if (endOfNamePosition>=0) {
              ...
              if (defines.containsKey(name)) {
                ...
                throw new ConfigDefineNotKnownException("Variable \""+name+"\" not defined", aLocation);
              }
            }
            else {
