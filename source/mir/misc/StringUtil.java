 ...
 *
 * @version $Id: StringUtil.java,v 1.23.2.6 2002/12/20 02:57:51 mh Exp $
...
public final class StringUtil {
  ...
  /**
   * Escapes characters that cannot be used in an SQL statement like the single
   * quote char. For postgres it relies on the double single quote to escape
   * a quote instead of the JDBC backslash which is broken.
   *
   * @return String the escaped SQL text.
   *
   */
  public static String JDBCescapeStringLiteral(String aText) {
    final char[] CHARACTERS_TO_ESCAPE = { '\'', '\\', '%', '_', '?' };
    final String[] ESCAPE_CODES = { "\'\'", "\\\\", "\\%", "\\_", "\\?" };

    return replaceStringCharacters(aText, CHARACTERS_TO_ESCAPE, ESCAPE_CODES);
  }
  ...
  /**
   * replaces a given set of characters in String aText with a given set of
   * Strings.
	 * @param aText the String to me modified
	 * @param aCharactersToReplace an array of characters to be replaced
	 * @param aStringsToSubstitute an array of Strings to replace the chars with.
  ...
  public static String replaceStringCharacters(String aText, char[] aCharactersToReplace, String[] aStringsToSubstitute) {
    ...
    do {
      nextPosition = indexOfCharacters(aText, aCharactersToReplace, position);
